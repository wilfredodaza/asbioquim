<?php if ($pdf): ?>
    ...
    <div class="campo_imprimir" style="width: 800px;background: white; padding: 20px 40px;">
<?php else: ?>
    <div class="campo_imprimir">
<?php endif ?>
	<div class="row">
		<div class="col s12 l12">
			<div id="campo_logo" align="right">
	            <!-- <img src="assets/img/blank.png" width="300" height="80" /> -->
	            <table width="100%" border="1" cellpadding="0" cellspacing="0" class="centered border">
	                <thead>
	                    <tr>
                            <?php if ($pdf): ?>
	                           <?= $pdf ? '<td': '<th' ?> width="170"><img src="./public/assets/img/logo.png" width="130" height="60" /><?= $pdf ? '</td>': '</th>' ?>
                            <?php else: ?> 
                               <?= $pdf ? '<td': '<th' ?> width="130" height="60"><img src="<?= base_url() ?>/assets/img/logo.png" width="150" height="40" /><?= $pdf ? '</td>': '</th>' ?>
                            <?php endif ?>
	                        <?= $pdf ? '<td': '<th' ?>><b><h1>Reporte de Ensayo </h1></b><?= $pdf ? '</td>': '</th>' ?>
	                        <?= $pdf ? '<td': '<th' ?> width="170"><h2 align="center"><br>Versi&oacute;n 03</h2><?= $pdf ? '</td>': '</th>' ?>
	                    </tr>
	                </thead>
	            </table>
	        </div>
	...
	<div class="row">
		<div class="col s12 l12">
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
				<thead>
			    	<tr>
			    		<td colspan="4" ><br><h2> <?= $aux_mensaje ?> No - <?= $certificado->certificado_nro ?> </h2><br></td>
			    	</tr>
			    </thead>
			    <tbody>
				    <tr>
	...
	<?php 
		$plantilla=procesar_registro_fetch('certificacion_vs_mensaje', 'id_certificacion', $certificado->certificado_nro);
	    
	     // firma de sistema
	    $firma	= procesar_registro_fetch('cms_firma', 'id_firma', $fila_plantilla->id_firma);
	    ...
	    $aux_nombre    = $fila_firma2->nombre;
	    $aux_cargo     = $fila_firma2->cargo;
	?>
	<br/>
	<!-- formateo deacuerdo a la plantilla -->
    <?php if($plantilla[0]->id_plantilla == 1 || $plantilla[0]->id_plantilla == 11 || $plantilla[0]->id_plantilla == 111): ?><!-- PLANTILLA STANDAR -->
        ...
        <!--TABLA DE RESULTADOS-->
        <br/>
        <table width="100%" class="centered border" border="1" cellpadding="0" cellspacing="0" >
            <thead>
                <tr>
                    <th colspan="<?= $conteo_columnas ?>"><i><h2>TABLA DE RESULTADOS </h2></i></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                ...
                <?php foreach ($ensayos as $key => $ensayo): ?>
                    ...
                    <tr>
                        <td class="left-align"><?= $aux_descripcion_ensayo ?></td>
                        ...
                        <?php foreach ($analisis_aux as $key => $analisis): ?>
                            ...
                            <?php else: ?>
                             ...
                             <?php
                                    $plantilla[0]->form_valo = max_valor($plantilla[0]->id_certificacion);
                                    $ensayo->resultado_mensaje = formateo_valores($ensayos2->resultado_mensaje, $plantilla[0]->form_valo);
                                ?>
    ...
    <?php elseif($plantilla[0]->id_plantilla == 2): ?> <!-- //PLANTILLA FROTIS -->
        <?php
        ...
        <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" >
            ...
            <tbody>
                <tr valign=middle class="center">
                    ...
                    <td width="100"><strong>C&oacute;digo AMC</strong></td>
    ...
    <?php elseif($plantilla[0]->id_plantilla == 5): ?>
        <table width="100%" border="1" align="center" cellpadding="0" cellspacing="0" >
            <thead>
                <tr>
                    <td colspan="8"><h2 align="center"><i>IDENTIFICACI&Oacute;N DE LA MUESTRA</i></h2></td>
    ...
    <?php endif ?>
    <?php if ($plantilla[0]->id_plantilla == 5): ?>
        <i class="icon">&#xf00c;</i> No present&oacute; crecimiento
    <?php endif ?>
        
    <?php $mensajes =  procesar_registro_fetch('certificacion_vs_mensaje', 'id_certificacion', $certificado->certificado_nro, 'id_mensaje_tipo', $tipo_mensajes); ?>
    <?php $resultado = procesar_registro_fetch('mensaje_resultado', 'id_mensaje', $mensajes[0]->id_mensaje_resultado) ?>
    <?php $resultado_observacion = procesar_registro_fetch('mensaje', 'id_mensaje', $mensajes[0]->id_mensaje_comentario) ?>
    <!-- // firma de sistema -->
    <?php $firma  = procesar_registro_fetch('cms_firma', 'id_firma', $plantilla[0]->id_firma); ?>
    ...
    <div class="row">
        <div class="col s12 l12">
            <table width="100%" cellpadding="0" cellspacing="0" >
                <thead>
                    <tr>
                        <th colspan="2"><h2><?= $resultado[0]->mensaje_titulo ?></h2></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td colspan="2"><?= $resultado_observacion[0]->mensaje_detalle ?> 
                            ...
                            * Confirme la validez de este documento ingresando a www.amc-laboratorios.com y el codigo <?= $certificado->clave_documento_pre ?>
                            <br/>
                            <i><b>AMC An&aacute;lisis de Colombia Ltda</b></i>
                            <br>
                            <?php if ($pdf): ?>
                                <img height="58" src="assets/img/firmas/<?= $aux_firma?>" />
                            <?php else: ?>
                                <img height="58" src="<?= base_url() ?>/assets/img/firmas/<?= $aux_firma?>" />    
                            <?php endif ?>
                        </td>
                    </tr>
                    <tr>
                        <td><i><b><?= $aux_nombre1 ?></b></i></td>
                        <td><i><b><?= $aux_nombre ?></b></i></td>
                    </tr>
                    <tr>
                        <td><i><b><?= $aux_cargo1 ?></b></i></td>
                        <td><i><b><?= $aux_cargo ?></b></i></td>
                    </tr>
                    <tr class="center">
